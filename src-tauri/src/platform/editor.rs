...
pub fn lookup_project_path(
    ...
    // list_projects() computes is_missing live; we use it only for path lookup.
...
/// `AppError::Io`.
pub async fn open_in_editor_impl(
    id: &str,
    path: PathBuf,
    app: &tauri::AppHandle,
) -> AppResult<()> {
...
mod tests {
    ...
    fn make_registry_with_project() -> (TempDir, TempDir, ProjectRegistry, String) {
    ...
    #[test]
    fn lookup_project_path_returns_path_for_known_id() {
        let (_config, proj_dir, registry, id) = make_registry_with_project();
    ...
    #[test]
    fn lookup_project_path_returns_not_found_for_unknown_id() {
        let (_config, _proj_dir, registry, _id) = make_registry_with_project();
    ...
    #[test]
    fn lookup_project_path_returns_not_found_for_empty_id() {
        let (_config, _proj_dir, registry, _id) = make_registry_with_project();
    ...
    #[test]
    fn lookup_project_path_finds_correct_project_among_multiple() {
        let config = TempDir::new().expect("config tempdir");
    ...
    /// missing projects.
    #[test]
    fn lookup_project_path_returns_path_for_is_missing_project() {
        let config = TempDir::new().expect("config tempdir");
        ...
        let registry = ProjectRegistry::load(config.path());
        let listed = registry.list_projects();
